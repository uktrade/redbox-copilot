...
def mock_embeddings(session_mocker, embedding_model):
    session_mocker.patch(
        "core_api.dependencies.get_embedding_model", return_value=embedding_model
    )
...
def test_make_chat_prompt_from_messages_runnable(mock_llm):
    ...
    response = chain.invoke(
        input={
            ...
            "chat_history": [
                (
                    {"text": str(i), "role": "user"}
                    if i % 2 == 0
                    else {"text": str(i), "role": "ai"}
                )
                for i in range(100)
            ],
        }
...
def test_rag_runnable(mock_llm, chunked_file, env):
    ...
    chain = build_retrieval_chain(
        llm=mock_llm, retriever=retriever, tokeniser=get_tokeniser(), env=env
    )
    ...
    assert {str(chunked_file.uuid)} == {
        chunk.metadata["parent_file_uuid"] for chunk in response["source_documents"]
    }


def test_coach_runnable(mock_llm, chunked_file, env):
    chain = build_coach_chain(llm=mock_llm, tokeniser=get_tokeniser(), env=env)
...
def test_condense_runnable(mock_llm, chunked_file, env):
    ...
    all_results_file_uuids = {
        chunk.metadata["parent_file_uuid"] for chunk in response["source_documents"]
    }
    assert {
        str(chunked_file.uuid)
    } == all_results_file_uuids, (
        f"Expected {str(chunked_file.uuid)} in {all_results_file_uuids}"
    )
...
def test_chat_with_documents_runnable(
    all_chunks_retriever, mock_llm, chunked_file, env
):
