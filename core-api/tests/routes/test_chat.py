...
EXPECTED_AVAILABLE_ROUTES = ("chat", "search", "info", "coach")
...
def mock_client(alice):
    chat_app.dependency_overrides[dependencies.get_llm] = mock_get_llm(
        [RAG_LLM_RESPONSE] * 32
    )
    chat_app.dependency_overrides[dependencies.get_all_chunks_retriever] = (
        lambda: mock_all_chunks_retriever(alice)
    )
    chat_app.dependency_overrides[dependencies.get_parameterised_retriever] = (
        lambda: mock_parameterised_retriever(alice)
    )
...
def mock_streaming_client():
    ...
    chat_app.dependency_overrides[semantic_routes.get_routable_chains] = lambda: {
...
def test_coach(mock_client, headers):

    message_history = [
        {"text": "What can I do for you?", "role": "system"},
        {"text": "Who put the ram in the rama lama ding dong?", "role": "user"},
        {"text": "The lama put the ram in rama", "role": "ai"},
        {"text": "@coach", "role": "user"},
    ]
    selected_files = [{"uuid": UPLOADED_FILE_UUID}]
    ...
    assert chat_response.output_text == RAG_LLM_RESPONSE
    assert (
        chat_response.route_name == ChatRoute.coach
    ), f"Expected route [{ChatRoute.coach}] received [{chat_response.route_name}]"
...
def test_rag_chat_streamed(mock_client, headers):
    ...
    with mock_client.websocket_connect("/chat/rag", headers=headers) as websocket:
        ...
        websocket.send_text(
            json.dumps(
                {"message_history": message_history, "selected_files": selected_files}
            )
        )
...
def test_available_tools(mock_client, headers):
    ...
    assert set(EXPECTED_AVAILABLE_ROUTES).issubset(
